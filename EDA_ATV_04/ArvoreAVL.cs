...
{
    public class ArvoreAVL : ArvoreBinaria
    {
        private No raiz;
		...
		private int BuscaAVL(No no, int k, ref int cont){
			cont++;
    		if (no.info == k)
       			return no.info;
    		if (no.info > k) return BuscaAVL(no.noEsquerdo, k, ref cont);
    		else 			 return BuscaAVL(no.noDireito, k, ref cont);
		}
		...
		// em ordem
		override public void PercorrerEmOrdem()
		{
        ...
        override public void Inserir(int x)
		{
			raiz = Inserir(raiz, x);
		}
		...
		override public void Remover(int x)
		{
			raiz = Remover(raiz, x);
		}
